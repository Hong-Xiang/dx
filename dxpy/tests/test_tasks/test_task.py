import unittest
from dxpy.tasks.task import TaskState, TaskStateCode, Task, TaskService

class TestTaskState(unittest.TestCase):
    def test_undifined(self):
        s = TaskState()
        assert s.format() == ['Undifined']
        

    def test_format(self):
        s0 = TaskState(TaskStateCode.WaitingToSubmit)
        assert s.format() == ['WaitingToSubmit']
        
        

# class TestTask(unittest.TestCase):
#     def test_str(self):
#         t = TaskService.new(Task, name='test_str', desc='Test task for str.')
#         assert str(t) == """<Task #1:
#         name: test_str,
#         workdir: None,
#         sub_tasks: None,
